...
export const featuredRaids: FeaturedRaid[] = [
	{
		...
		artPath:
			...
			'/raids/march_on_queldanas_styled_16x9.png',
		leader: 'Kaelthas',
		date: '25 мая',
		time: '20:00',
		party: '23/30',
	...
	},
	{
		...
		artPath:
			...
			'/raids/the_dreamrift_styled_16x9.png',
		leader: 'Dreamseer',
		date: '26 мая',
		time: '21:00',
		party: '27/30',
	...
	{
		slug: 'sporefall',
		title: raidBySlug.get('sporefall')?.name ?? 'Споропад',
		artPath:
			raidBySlug.get('sporefall')?.artPath ??
			'/raids/sporefall_styled_16x9.png',
		leader: 'Rotmire',
		date: '27 мая',
		time: '20:00',
		party: '20/20',
		slotPrice: '30 000 золота',
		difficulties: ['LFR', 'NORMAL', 'HEROIC', 'MYTHIC'],
	},
]
...
export function getFeaturedRaids(locale: AppLocale): FeaturedRaid[] {
	const source = featuredRaids

	return source.map(raid => {
		const currentRaid = raidBySlug.get(raid.slug)

		return {
			...raid,
			title: currentRaid ? getLocalizedRaidName(currentRaid, locale) : raid.title,
		}
	})
}

export function getActivityItems(locale: AppLocale): ActivityItem[] {
	return [
		{ label: t(locale, 'home.activityKeys'), icon: KeyRound, tone: 'green' },
		{
			label: t(locale, 'home.activityMounts'),
			icon: WandSparkles,
			tone: 'violet',
		},
		{
			label: t(locale, 'home.activityResources'),
			icon: Gem,
			tone: 'blue',
		},
		{
			label: t(locale, 'home.activityAchievements'),
			icon: Trophy,
			tone: 'gold',
		},
		{
			label: t(locale, 'home.activityTransmog'),
			icon: Sparkles,
			tone: 'purple',
		},
		{
			label: t(locale, 'home.activityRaiderIo'),
			icon: Dice5,
			tone: 'silver',
		},
	]
...
export function getFeatureTiles(locale: AppLocale): FeatureTile[] {
	return [
		{
			title: t(locale, 'home.featurePlanningTitle'),
			text: t(locale, 'home.featurePlanningText'),
			icon: CalendarDays,
			tone: 'calendar',
		},
		{
			title: t(locale, 'home.featureGoldTitle'),
			text: t(locale, 'home.featureGoldText'),
			icon: Coins,
			tone: 'gold',
		},
		{
			title: t(locale, 'home.featureRolesTitle'),
			text: t(locale, 'home.featureRolesText'),
			icon: UsersRound,
			tone: 'team',
		},
	]
...
export function getFooterLinks(locale: AppLocale) {
	return [
		t(locale, 'home.footerAbout'),
		t(locale, 'home.footerRules'),
		t(locale, 'home.footerSupport'),
		t(locale, 'home.footerFaq'),
		t(locale, 'home.footerContacts'),
	]
}

export function getSocialLinks(locale: AppLocale) {
	return [
		{ label: 'Discord', icon: Crown },
		{ label: 'VK', icon: UsersRound },
		{ label: 'Telegram', icon: Search },
		{
			label: locale === 'ru' ? 'Компас' : 'Compass',
			icon: Sparkles,
		},
	]
}
