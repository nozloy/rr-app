...
describe("raid boss localization", () => {
  ...
  it("falls back to normalized alias matches", () => {
    ...
    expect(localizeRaidBossName({ name: "Belo’ren, Child of Al’ar" }, "ru")).toBe(
      "Бело'рен, Дитя Ал'ара",
    );
    expect(localizeRaidBossName({ name: "Chimaerus, the Undreamt God" }, "ru")).toBe(
      "Химерий, Неприснившийся Бог",
    );
  });
